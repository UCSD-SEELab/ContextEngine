...
##    outputVector = []
    ...
    def __init__(self,
                 ...
                 inputClassifiers,
                 appFieldsDict):
        super(Tesla, self).__init__(numInputs,
                     outputClassifier,
                     inputClassifiers,
                     appFieldsDict)
        if "complexity" in appFieldsDict:
            self.functionOrder = self.customFieldsDict["complexity"]
    ...
    def addSingleObservation(self, newInputObs, newOutputObs):
        if (len(newInputObs) == self.numInputs
            ...
            newInputObs = np.insert(newInputObs, 0, 1)
    ...
    def normalizeData(self):
        if self.interface.outObj.norm == 'lin':
            raise ValueError ('Normalization is not implemented for Tesla yet.')
        # input normalization
